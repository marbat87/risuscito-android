...
public class FavouritesActivity extends Fragment
  ...
  public View onCreateView(
        ...
        new View.OnClickListener() {
          ...
          public void onClick(View v) {
            new SimpleDialogFragment.Builder(
                    (AppCompatActivity) getActivity(), FavouritesActivity.this, "FAVORITES_RESET")
    ...
    OnClickListener<SimpleItem> mOnPreClickListener =
    ...
    OnLongClickListener<SimpleItem> mOnPreLongClickListener =
        new OnLongClickListener<SimpleItem>() {
          @Override
          public boolean onLongClick(
              View view, IAdapter<SimpleItem> iAdapter, SimpleItem item, int i) {
...
//    cantoAdapter.saveInstanceState(savedInstanceState);
    ...
    mUndoHelper =
        new UndoHelper(
            cantoAdapter,
            new UndoHelper.UndoListener<SimpleItem>() {
              @Override
              public void commitRemove(
                  Set<Integer> set, ArrayList<FastAdapter.RelativeInfo<SimpleItem>> arrayList) {
                ...
                for (FastAdapter.RelativeInfo<SimpleItem> item : arrayList) {
                  ...
                  new Thread(
                          new Runnable() {
                            ...
                            public void run() {
                              FavoritesDao mDao =
                                  RisuscitoDatabase.getInstance(getContext()).favoritesDao();
                              mDao.removeFavorite(mItem.getId());
                            }
                          })
                      .start();
                }
                //                mNoFavorites.setVisibility(cantoAdapter.getAdapterItemCount() > 0
  ...
  //    private void updateFavouritesList(List<Canto> data) {
  //
  ...
  //
  ////        List<Canto> favorites = mDao.getFavorites();
  //
  ...
  //        };
  //
  //        OnLongClickListener<SimpleItem> mOnPreLongClickListener = new
  // OnLongClickListener<SimpleItem>() {
  //            @Override
  //            public boolean onLongClick(View view, IAdapter<SimpleItem> iAdapter, SimpleItem
  // item, int i) {
  //                if (mMainActivity.getMaterialCab().isActive())
  //                    return true;
  //                if (!mMainActivity.isOnTablet() && mMainActivity.getAppBarLayout() != null)
  ...
  // ArrayList<FastAdapter.RelativeInfo<SimpleItem>> arrayList) {
  //                Log.d(TAG, "commitRemove: " + arrayList.size());
  ////                SQLiteDatabase db = listaCanti.getReadableDatabase();
  ...
  //                    CantoDao mDao = RisuscitoDatabase.getInstance(getContext()).cantoDao();
  //                    Canto cantoToUpdate = mDao.getCantoById(mItem.getId());
  //                    cantoToUpdate.favorite = 0;
  //                    mDao.updateCanto(cantoToUpdate);
  //
  //
  //                }
  ////                db.close();
      ...
      case "FAVORITES_RESET":
        ...
        new Thread(
                new Runnable() {
                  @Override
                  public void run() {
                    FavoritesDao mDao = RisuscitoDatabase.getInstance(getContext()).favoritesDao();
                    mDao.resetFavorites();
                  }
                })
            .start();

...
interface ArgomentiDao {
    ...
    @get:Query("SELECT C.id, C.pagina, C.titolo, C.source, C.color, B.idArgomento, A.nomeArgomento FROM nomeargomento A, argomento B, canto c WHERE A.idArgomento = B.idArgomento AND b.idCanto = c.id ORDER BY A.nomeArgomento ASC, C.titolo ASC")

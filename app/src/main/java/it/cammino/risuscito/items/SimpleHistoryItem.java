...
public class SimpleHistoryItem
    extends AbstractItem<SimpleHistoryItem, SimpleHistoryItem.ViewHolder> {
    ...
    if (timestamp != null) {
      ...
      DateFormat df =
          DateFormat.getDateTimeInstance(
              DateFormat.SHORT, DateFormat.MEDIUM, ctx.getResources().getConfiguration().locale);
      String tempTimestamp;

      Date dateTimestamp = new Date(Long.parseLong(timestamp.getText().toString()));
      if (df instanceof SimpleDateFormat) {
        SimpleDateFormat sdf = (SimpleDateFormat) df;
        String pattern = sdf.toPattern().replaceAll("y+", "yyyy");
        sdf.applyPattern(pattern);
        //                tempTimestamp =
        // sdf.format(Timestamp.valueOf(timestamp.getText().toString()));
        tempTimestamp = sdf.format(dateTimestamp);
      } else
        //                tempTimestamp =
        // df.format(Timestamp.valueOf(timestamp.getText().toString()));
        tempTimestamp = df.format(dateTimestamp);
      //            viewHolder.mTimestamp.setText(ctx.getString(R.string.last_open_date,
      // tempTimestamp));
      viewHolder.mTimestamp.setText(tempTimestamp);
      viewHolder.mTimestamp.setVisibility(View.VISIBLE);
    } else viewHolder.mTimestamp.setVisibility(View.GONE);

...
public class Playback
  ...
  public void play(MediaSessionCompat.QueueItem item) {
    ...
    if (mState == PlaybackStateCompat.STATE_PAUSED && !mediaHasChanged && mMediaPlayer != null) {
      ...
      try {
        ...
        if (source.startsWith("http")) mMediaPlayer.setDataSource(source);
        else {
        ...
        if (source.startsWith("http")) mWifiLock.acquire();
        else {
          if (mWifiLock.isHeld()) mWifiLock.release();
        }
  ...
  public long getDuration() {
    if (mMediaPlayer != null && mState == PlaybackStateCompat.STATE_PLAYING) {
  ...
  private void setStreamType() {
    if (LUtils.Companion.hasL()) setStreamTypeLollipop();
    else setStreamTypeLegacy();
  }

...
open class GenericIndexViewModel(application: Application) : AndroidViewModel(application) {
                        ...
                        if (idListaClick != ID_FITTIZIO && idListaClick >= 100) {
                            ...
                            if (liste[idListaClick]
                                ...
                                if (liste[idListaClick]
                                    ...
                                    ListeUtils.manageReplaceDialog(
    ...
    fun popupMenu(
        ...
        val mPopupMenu = PopupMenu(activity, view, Gravity.END)
        ...
        mPopupMenu.setOnMenuItemClickListener {
            when (it.itemId) {
                R.id.add_to_favorites -> {
                    activity.addToFavorites(idDaAgg, true)
                    true
                }
                R.id.add_to_p_iniziale -> {
                    addToListaNoDup(1, 1, activity, dialogTag2)
                    true
                }
                R.id.add_to_p_prima -> {
                    addToListaNoDup(1, 2, activity, dialogTag2)
                    true
                }
                R.id.add_to_p_seconda -> {
                    addToListaNoDup(1, 3, activity, dialogTag2)
                    true
                }
                R.id.add_to_p_terza -> {
                    addToListaNoDup(1, 4, activity, dialogTag2)
                    true
                }
                R.id.add_to_p_pace -> {
                    addToListaNoDup(1, 6, activity, dialogTag2)
                    true
                }
                R.id.add_to_p_fine -> {
                    addToListaNoDup(1, 5, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_iniziale -> {
                    addToListaNoDup(2, 1, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_seconda -> {
                    addToListaNoDup(2, 6, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_pace -> {
                    addToListaNoDup(2, 2, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_offertorio -> {
                    addToListaNoDup(2, 8, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_santo -> {
                    addToListaNoDup(2, 7, activity, dialogTag2)
                    true
                }
                R.id.add_to_e_pane -> {
                    activity.addToListaDup(2, 3, idDaAgg)
                    true
                }
                R.id.add_to_e_vino -> {
                    activity.addToListaDup(2, 4, idDaAgg)
                    true
                }
                R.id.add_to_e_fine -> {
                    addToListaNoDup(2, 5, activity, dialogTag2)
                    true
                }
                else -> {
                    idListaClick = it.groupId
                    ...
                    listePersonalizzate?.let { liste ->
                        if (idListaClick != ID_FITTIZIO && idListaClick >= 100) {
                            ...
                            if (liste[idListaClick]
                                ...
                                activity.updateListaPersonalizzata(liste[idListaClick])
                            } else {
                                if (liste[idListaClick]
                                        .lista
                                        ?.getCantoPosizione(idPosizioneClick) == (idDaAgg).toString()
    ...
    private fun addToListaNoDup(
        ...
        idListaDaAgg = idLista
        posizioneDaAgg = listPosition
        activity.addToListaNoDup(idLista, listPosition, idDaAgg, dialogTag)
    }
